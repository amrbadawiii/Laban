    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div class="bg-white p-4 shadow rounded-lg">
            ...
            <form action="{{ route('inbounds.update', $inbound['id']) }}" method="POST">
                ...
                <!-- Product Selection -->
                <x-select name="product_id" label="{{ __('messages.product') }}" :options="$products->pluck('name', 'id')" :selected="$inbound['product_id']"
                    required />

                <!-- Measurement Unit Selection -->
                <x-select name="measurement_unit_id" label="{{ __('messages.measurement_unit') }}" :options="$measurementUnits->pluck('abbreviation', 'id')"
                ...
                <!-- Supplier Selection -->
                <x-select name="supplier_id" label="{{ __('messages.supplier') }}" :options="$suppliers->pluck('name', 'id')" :selected="$inbound['supplier_id']"
                    :nullable="true" />

                <!-- Warehouse Selection -->
                <x-select name="warehouse_id" label="{{ __('messages.warehouse') }}" :options="$warehouses->pluck('name', 'id')" :selected="$inbound['warehouse_id']"
                    required />
